...
class BrunnCoreImport {
	...
	public function set_import_url() {
		$params = BrunnCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
				...
				if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
					...
					switch ($_POST['options']['action']):
						...
						case 'menu-settings':
							...
							break;
						case 'rev-slider':
							if (brunn_core_is_revolution_slider_installed()) {
								$this->rev_slider_import($demo);
							} else {
								$this->set_status('success');
								$this->set_data('type', 'options');
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'brunn-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!BrunnCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									brunn_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'brunn-core'));
									exit;
								}
							}
			...
			brunn_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'brunn-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $brunn_widget_id => $brunn_widget_data) {
				update_option('widget_' . $brunn_widget_id, $brunn_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'brunn-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$brunn_sidebars = get_option( "sidebars_widgets" );
		unset( $brunn_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$brunn_sidebars = array_merge( (array) $brunn_sidebars, (array) $data['sidebars'] );
			unset( $brunn_sidebars['wp_inactive_widgets'] );
			$brunn_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $brunn_sidebars );
			$brunn_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $brunn_sidebars );
			return true;
		} else {
			return false;
		}
	...
	public function import_options( $file ) {
		...
		$current_options = get_option(BRUNN_CORE_OPTIONS_NAME);
		if($options){
			if($current_options != $options) {
				$result = update_option(BRUNN_CORE_OPTIONS_NAME, $options);
				if ($result) {
					$this->update_options_after_import($file);
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'brunn-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'brunn-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'brunn-core'));
			}
		}

		if(brunn_select_is_elementor_installed()){
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_brunn_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'brunn-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'brunn-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'brunn-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'brunn-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'brunn-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( BRUNN_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//		}
//
		if(brunn_core_is_woocommerce_installed()) {
		...
		$brunn_import = new WP_Import();
		set_time_limit( 0 );

		$brunn_import->fetch_attachments = $attachments;
		$returned_value                  = $brunn_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'brunn-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'brunn-core' ) . ' ' . $xml );
	...
	public function rev_sliders() {
		$rev_sldiers = array(
			...
			'landing-slide-1.zip',
			'home-4-slider-1.zip',
			'landing-inner-rev-1.zip',
			'landing-inner-rev-2.zip',
			'landing-inner-rev-3.zip',
            'home-4-slider-2.zip',
			'landing-slide-elementor.zip'
		);
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'brunn-core'));
					exit;
				}
			}
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'brunn-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'brunn-core'));
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( BRUNN_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( BRUNN_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return BRUNN_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'qodef_cd_import_nonce' ) ) {
				...
				$html = brunn_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					brunn_core_ajax_status( 'success', '', $html);
				} else {
					brunn_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'brunn-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = BrunnCoreSystemInfoPage::get_instance()->get_system_info();
