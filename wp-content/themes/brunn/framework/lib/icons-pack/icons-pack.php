...
foreach ( glob( SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( brunn_select_is_customizer_item_enabled( $module_load, 'brunn_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'brunn_select_get_icon_pack_collections' ) ) {
	function brunn_select_get_icon_pack_collections() {
		$options = apply_filters( 'brunn_select_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'brunn_select_get_icon_pack_collections' );
}

if ( ! function_exists( 'brunn_select_get_icon_pack_options' ) ) {
	function brunn_select_get_icon_pack_options() {
		$options = apply_filters( 'brunn_select_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'brunn_select_get_icon_pack_options' );
}

if ( ! function_exists( 'brunn_select_get_icon_pack_social_options' ) ) {
	function brunn_select_get_icon_pack_social_options() {
		$options = apply_filters( 'brunn_select_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'brunn_select_get_icon_pack_social_options' );
}

/*
 * Class: BrunnSelectClassIconCollections
 */
class BrunnSelectClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = brunn_select_get_icon_pack_collections();
		$this->iconPackOptions       = brunn_select_get_icon_pack_options();
		$this->iconPackSocialOptions = brunn_select_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = brunn_select_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'brunn' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'brunn' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'brunn' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'brunn' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'brunn' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'brunn' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'brunn' )       => 'fa-lg',
			esc_html__( 'Small', 'brunn' )      => 'fa-2x',
			esc_html__( 'Medium', 'brunn' )     => 'fa-3x',
			esc_html__( 'Large', 'brunn' )      => 'fa-4x',
			esc_html__( 'Very Large', 'brunn' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iBrunnSelectInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'brunn' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'brunn' ) ] = '';
